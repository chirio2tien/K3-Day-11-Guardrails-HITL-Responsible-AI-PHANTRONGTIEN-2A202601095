...
class MonitoringAlert:
    ...
    def check_metrics(self) -> list[Alert]:
        """Compute rates and raise alerts when a threshold is exceeded.

        MONITORING & INCIDENT RESPONSE: a spike in block rate, repeated
        rate-limit hits, or a rising judge-failure rate are early signals of an
        attack campaign — surface them instead of silently dropping requests.
        """
        self.alerts = []
        ...
        if block_rate > self.block_rate_threshold:
            self.alerts.append(Alert(
                ...
                message=(
                    f"Block rate {block_rate:.0%} exceeds "
        ...
        if self.rate_limit_hits > self.rate_limit_hit_threshold:
            self.alerts.append(Alert(
                ...
                message=(
                    f"{self.rate_limit_hits} rate-limit hits exceed "
                    f"{self.rate_limit_hit_threshold} — possible flooding/DoS."
                ),
            ))
        ...
        if judge_fail_rate > self.judge_fail_rate_threshold:
            self.alerts.append(Alert(
                ...
                message=(
                    f"Judge failure rate {judge_fail_rate:.0%} exceeds "
                    f"{self.judge_fail_rate_threshold:.0%} — model may be compromised."
                ),
            ))
